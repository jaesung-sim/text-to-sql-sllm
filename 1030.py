...
if sys.argv[1] == "--dataset" and sys.argv[3] == "--output":
    ...
    OUTPUT_FILE = os.path.join(script_dir, "1030.txt")  # 출력 파일 경로
...
# 모델과 토크나이저를 초기화합니다.
tokenizer = AutoTokenizer.from_pretrained("VictorDCh/Llama-3-8B-Instruct-spider")
model = AutoModelForCausalLM.from_pretrained("VictorDCh/Llama-3-8B-Instruct-spider")
...
def generate_text(prompt):
    result = pipe(prompt, max_length=600, num_return_sequences=1, do_sample=True, temperature=0.6, top_p=0.9,truncation=True)
    return result[0]['generated_text']




def debug_text(prompt):
    result = pipe(prompt, max_length=600, num_return_sequences=1, do_sample=True, temperature=0.6, top_p=0.9,truncation=True)
    return result[0]['generated_text']
...
def extract_sql_query(text):
    # ```sql로 시작하고 ```로 끝나는 구문을 먼저 제거한 후 SELECT~ 패턴을 추출
    sql_pattern = re.compile(r"```sql\s*(SELECT.*?);?\s*```", re.DOTALL)
    match = sql_pattern.search(text)
    if match:
        sql_query = match.group(1)  # 첫 번째 매칭된 그룹만 추출 (```sql 마커 제외)
        return sql_query.strip()  # 양끝 공백 제거 후 반환
    return ""
