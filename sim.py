...
if sys.argv[1] == "--dataset" and sys.argv[3] == "--output":
    ...
    DATASET = os.path.join(dataset_folder, "dev2.json")  # 데이터셋 파일 경로
    OUTPUT_FILE = os.path.join(script_dir, "3.txt")  # 출력 파일 경로
...
# 텍스트 생성을 위한 Ollama 함수
def ollama_generation(prompt):
    # 주어진 프롬프트를 기반으로 텍스트를 생성하는 함수입니다.
    #print(f"Generating text for prompt: {prompt[:100]}...")  # 프롬프트의 앞 100자만 출력
    generated_text = ""  # 결과 텍스트를 저장할 변수
    for chunk in llm.stream(prompt):  # 모델이 생성하는 텍스트 스트림을 받습니다.
        generated_text += chunk  # 각 부분을 결합합니다.
    #print(f"Generated text: {generated_text[:100]}...")  # 결과의 앞 100자만 출력
    return generated_text  # 최종 텍스트를 반환합니다.


# 데이터 로드 함수 (JSON 형식으로 데이터셋 로드)
def load_data(DATASET):
    # JSON 형식의 데이터셋을 로드하는 함수입니다.
    #print(f"Loading dataset from: {DATASET}")
    return pd.read_json(DATASET)  # pandas를 사용하여 JSON 데이터를 읽어옵니다.
...
def easy_prompt_maker(test_sample_text, database, schema_links):
    # EASY 복잡도 수준의 SQL 쿼리를 생성하기 위한 프롬프트를 만드는 함수입니다.
    #print(f"Creating easy prompt for question: {test_sample_text}")
...
# SQL 디버그 함수 (GPT-4를 Llama3로 변경)
def debuger(test_sample_text, database, sql):
    # SQL 쿼리의 오류를 디버깅하기 위한 함수입니다.
    #print(f"Debugging SQL for question: {test_sample_text}")
...
"""
    fields = find_fields_MYSQL_like(database)  # 필드 정보를 가져옵니다.
    fields += "Foreign_keys = " + find_foreign_keys_MYSQL_like(database) + '\n'  # 외래 키 정보를 가져옵니다.
    fields += "Primary_keys = " + find_primary_keys_MYSQL_like(database)  # 기본 키 정보를 가져옵니다.
    prompt = instruction + fields + '#### Question: ' + test_sample_text + '\n#### SQLite SQL QUERY\n' + sql +'\n#### SQLite FIXED SQL QUERY\nSELECT'  # 최종 프롬프트를 생성합니다.
    prompt += "\nPlease provide the final answer SQL query in the following format:\nThe answer is: <SQL Query>"
    return prompt  # 생성된 프롬프트를 반환합니다.


# 데이터베이스 스키마 및 외래 키 설정
def creatiing_schema(DATASET_JSON):
    # 주어진 데이터셋에서 스키마 정보를 생성하는 함수입니다.
    #print(f"Creating schema from dataset: {DATASET_JSON}")
...
if __name__ == '__main__':
    ...
    for index, row in val_df.iterrows():
        ...
        while classification is None:
            try:
                classification = ollama_generation(
                    classification_prompt_maker(row['question'], row['db_id'], schema_links[1:]))  # 질문 분류hard_prompt_maker()
            except Exception as e:
                time.sleep(3)  # 3초 대기
        ...
        while debugged_SQL is None:
            try:
                debugged_SQL = ollama_generation(debuger(row['question'], row['db_id'], SQL)).replace("\n", " ")  # 생성된 SQL 쿼리 디버깅
            except Exception as e:
                time.sleep(3)  # 3초 대기
                pass
        ...
        CODEX.append([row['question'], SQL, row['query'], row['db_id']])  # 결과 리스트에 추가

    
    df = pd.DataFrame(CODEX, columns=['NLQ', 'PREDICTED SQL', 'GOLD SQL', 'DATABASE'])  # 결과를 데이터프레임으로 변환
    results = df['PREDICTED SQL'].tolist()  # 예측된 SQL 쿼리 리스트를 생성
