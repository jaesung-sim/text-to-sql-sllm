...
#----------------------------------------------------prompts-----------------------------------------------
# 여기서 사용하는 여러 SQL 프롬프트 템플릿을 정의합니다.
# 이는 질문에 대해 SQL 쿼리를 생성할 때 사용됩니다.
# schema_linking_prompt: 데이터베이스 스키마와 외래 키를 기반으로 SQL 쿼리를 생성하는 데 도움을 줍니다.
# classification_prompt: SQL 쿼리를 복잡도(EASY, NON-NESTED, NESTED)에 따라 분류하는 데 사용됩니다.
# easy_prompt, medium_prompt, hard_prompt: 각 복잡도 수준에 따라 SQL 쿼리를 생성하는 데 사용됩니다.
...
# 기본 인자 설정
if len(sys.argv) < 5:
    # 실행 시 입력된 인자가 부족할 경우 기본 인자를 설정합니다.
    sys.argv = ["script_name", "--dataset", "spider", "--output", "predicted_sql.txt"]

# 입력 파라미터 검증 (dataset과 output 파일 확인)
if sys.argv[1] == "--dataset" and sys.argv[3] == "--output":
    # 현재 스크립트 파일의 디렉토리를 기준으로 spider 폴더의 절대 경로를 생성
    script_dir = os.path.dirname(os.path.abspath(__file__))  # 현재 스크립트의 디렉토리 경로를 가져옵니다.
    dataset_folder = os.path.join(script_dir, "spider")  # spider 폴더 경로를 설정합니다.
    
    # 데이터셋의 경로와 출력 파일의 경로를 설정합니다.
    DATASET_SCHEMA = os.path.join(dataset_folder, "tables.json")  # 스키마 파일 경로
    DATASET = os.path.join(dataset_folder, "dev.json")  # 데이터셋 파일 경로
    OUTPUT_FILE = os.path.join(script_dir, "codellstral_q4.txt")  # 출력 파일 경로
    TIME_OUTPUT_FILE = os.path.join(script_dir,"codestral_q4-detail.txt")
    
    # 파일 존재 여부 확인
    if not os.path.exists(DATASET_SCHEMA):
        print(f"Error: Schema file '{DATASET_SCHEMA}' does not exist.")
        #pass  # 에러 출력하지 않음
    if not os.path.exists(DATASET):
        print(f"Error: Dataset file '{DATASET}' does not exist.")
        #pass  # 에러 출력하지 않음
else:
...
def hard_prompt_maker(test_sample_text, database, schema_links, sub_questions):
    # HARD 복잡도 수준의 SQL 쿼리를 생성하기 위한 프롬프트를 만드는 함수입니다.
    #print(f"Creating hard prompt for question: {test_sample_text}")
    ...
    fields = find_fields_MYSQL_like("college_2")  # "college_2" 데이터베이스에 대한 필드 정보를 가져옵니다.
    fields += "Foreign_keys = " + find_foreign_keys_MYSQL_like("college_2") + '\n'  # "college_2"의 외래 키 정보를 가져옵니다.
    fields += find_fields_MYSQL_like(database)  # 주어진 데이터베이스의 필드 정보를 가져옵니다.
    fields += "Foreign_keys = " + find_foreign_keys_MYSQL_like(database) + '\n'  # 외래 키 정보를 가져옵니다.
    stepping = f'''\nA: Let's think step by step. "{test_sample_text}" can be solved by knowing the answer to the following sub-question "{sub_questions}".'''  # 질문 해결을 위한 단계적 접근을 추가합니다.
    fields += "\n"
    prompt = instruction + fields + hard_prompt + 'Q: "' + test_sample_text + '"' + '\nschema_links: ' + schema_links + stepping +'\nThe SQL query for the sub-question"'  # 최종 프롬프트를 생성합니다.
    return prompt  # 생성된 프롬프트를 반환합니다.

def medium_prompt_maker(test_sample_text, database, schema_links):
    # MEDIUM 복잡도 수준의 SQL 쿼리를 생성하기 위한 프롬프트를 만드는 함수입니다.
    #print(f"Creating medium prompt for question: {test_sample_text}")
...
def classification_prompt_maker(test_sample_text, database, schema_links):
    # SQL 쿼리의 복잡도를 분류하기 위한 프롬프트를 만드는 함수입니다.
    #print(f"Creating classification prompt for question: {test_sample_text}")
...
def schema_linking_prompt_maker(test_sample_text, database):
    # 스키마 링크를 생성하기 위한 프롬프트를 만드는 함수입니다.
    #print(f"Creating schema linking prompt for question: {test_sample_text}")
    instruction = "# Find the schema_links for generating SQL queries for each question based on the database schema and Foreign keys.\n"  # 기본 설명을 추가합니다.
    fields = find_fields_MYSQL_like(database)  # 필드 정보를 가져옵니다.
    foreign_keys = "Foreign_keys = " + find_foreign_keys_MYSQL_like(database) + '\n'  # 외래 키 정보를 가져옵니다.
    prompt = instruction + schema_linking_prompt + fields + foreign_keys + 'Q: "' + test_sample_text + """"\nA: Let’s think step by step."""  # 최종 프롬프트를 생성합니다.
    return prompt  # 생성된 프롬프트를 반환합니다.


def find_fields_MYSQL_like(db_name):
    # 주어진 데이터베이스에서 사용 가능한 필드를 찾는 함수입니다.
    #print(f"Finding fields for database: {db_name}")
    df = spider_schema[spider_schema['Database name'] == db_name]  # 해당 데이터베이스의 스키마를 가져옵니다.
    df = df.groupby(' Table Name')  # 테이블별로 그룹화합니다.
    output = ""
    for name, group in df:
        output += "Table " + name + ', columns = ['  # 테이블 이름을 추가합니다.
        for index, row in group.iterrows():
            output += row[" Field Name"] + ','  # 각 필드 이름을 추가합니다.
        output = output[:-1]  # 마지막 쉼표 제거
        output += "]\n"
    return output  # 최종 결과를 반환합니다.

def find_foreign_keys_MYSQL_like(db_name):
    # 주어진 데이터베이스에서 외래 키를 찾는 함수입니다.
    #print(f"Finding foreign keys for database: {db_name}")
    df = spider_foreign[spider_foreign['Database name'] == db_name]  # 해당 데이터베이스의 외래 키 정보를 가져옵니다.
    output = "["
    for index, row in df.iterrows():
        output += row['First Table Name'] + '.' + row['First Table Foreign Key'] + " = " + row['Second Table Name'] + '.' + row['Second Table Foreign Key'] + ','  # 외래 키 정보를 추가합니다.
    output = output[:-1] + "]"  # 마지막 쉼표 제거
    return output  # 최종 결과를 반환합니다.

def find_primary_keys_MYSQL_like(db_name):
    # 주어진 데이터베이스에서 기본 키를 찾는 함수입니다.
    #print(f"Finding primary keys for database: {db_name}")
    df = spider_primary[spider_primary['Database name'] == db_name]  # 해당 데이터베이스의 기본 키 정보를 가져옵니다.
    output = "["
    for index, row in df.iterrows():
        output += row['Table Name'] + '.' + row['Primary Key'] + ','  # 기본 키 정보를 추가합니다.
    output = output[:-1]  # 마지막 쉼표 제거
    output += "]\n"
    return output  # 최종 결과를 반환합니다.


# 데이터베이스 스키마 및 외래 키 설정
def creatiing_schema(DATASET_JSON):
    schema_df = pd.read_json(DATASET_JSON)  # JSON 데이터를 pandas 데이터프레임으로 로드합니다.
    schema_df = schema_df.drop(['column_names','table_names'], axis=1)  # 사용하지 않는 열을 제거합니다.
    schema = []  # 스키마 정보를 저장할 리스트
    f_keys = []  # 외래 키 정보를 저장할 리스트
    p_keys = []  # 기본 키 정보를 저장할 리스트
    for index, row in schema_df.iterrows():
        tables = row['table_names_original']  # 테이블 이름을 가져옵니다.
        col_names = row['column_names_original']  # 컬럼 이름을 가져옵니다.
        col_types = row['column_types']  # 컬럼 타입을 가져옵니다.
        foreign_keys = row['foreign_keys']  # 외래 키 정보를 가져옵니다.
        primary_keys = row['primary_keys']  # 기본 키 정보를 가져옵니다.
        for col, col_type in zip(col_names, col_types):
            index, col_name = col
            if index == -1:
                for table in tables:
                    schema.append([row['db_id'], table, '*', 'text'])  # 스키마 정보를 추가합니다.
            else:
                schema.append([row['db_id'], tables[index], col_name, col_type])  # 스키마 정보를 추가합니다.
        for primary_key in primary_keys:
            index, column = col_names[primary_key]
            p_keys.append([row['db_id'], tables[index], column])  # 기본 키 정보를 추가합니다.
        for foreign_key in foreign_keys:
            first, second = foreign_key
            first_index, first_column = col_names[first]
            second_index, second_column = col_names[second]
            f_keys.append([row['db_id'], tables[first_index], tables[second_index], first_column, second_column])  # 외래 키 정보를 추가합니다.
    spider_schema = pd.DataFrame(schema, columns=['Database name', ' Table Name', ' Field Name', ' Type'])  # 스키마 데이터프레임을 생성합니다.
    spider_primary = pd.DataFrame(p_keys, columns=['Database name', 'Table Name', 'Primary Key'])  # 기본 키 데이터프레임을 생성합니다.
    spider_foreign = pd.DataFrame(f_keys,
                        columns=['Database name', 'First Table Name', 'Second Table Name', 'First Table Foreign Key',
                                 'Second Table Foreign Key'])  # 외래 키 데이터프레임을 생성합니다.
    #print("Schema creation complete.")
    return spider_schema,spider_primary,spider_foreign  # 생성된 스키마, 기본 키, 외래 키 데이터를 반환합니다.
...
def debuger(test_sample_text, database, sql):
    instruction = """#### Follow these instructions strictly:
    For the given question, use the provided tables, columns, foreign keys, and primary keys to fix the given SQLite SQL QUERY for any issues. If there are any problems, fix them. If there are no issues, return the SQLite SQL QUERY as is, ending with a semicolon.
#### Ensure that the SQL query does not repeat '######' or any extra characters after the semicolon.
#### Use the following instructions for fixing the SQL QUERY:
1) Use the database values that are explicitly mentioned in the question.
2) Pay attention to the columns that are used for the JOIN by using the Foreign_keys.
3) Use DESC and DISTINCT when needed.
4) Ensure the query ends with a semicolon.
5) Avoid redundant lines or comments following the SQL query.
#### For the given question, generate the SQL query based on the provided schema links.
"At the end of your response, please provide the SQL query in the following format:\n"
"I think final SQL QUERY: [Your SQL query here]"
"""
    fields = find_fields_MYSQL_like(database)
    fields += "Foreign_keys = " + find_foreign_keys_MYSQL_like(database) + '\n'
    fields += "Primary_keys = " + find_primary_keys_MYSQL_like(database)
    prompt = instruction + fields + '#### Question: ' + test_sample_text + '\n#### SQLite SQL QUERY\n' + sql + '\n#### SQLite FIXED SQL QUERY I think final SQL QUERY: SELECT\nSELECT'
    return prompt



def ollama_generation(prompt):
    ...
    # 'choices' 리스트에서 첫 번째 메시지의 'content' 필드를 추출
...
def ollama_generation(prompt):
    ...
    # 응답 객체 출력 (이미 확인된 내용을 바탕으로 필요한 정보를 추출)
...
def ollama_debug(prompt):
    ...
    # 'choices' 속성을 통해 첫 번째 메시지의 'content'에 접근
    if hasattr(response, 'choices') and len(response.choices) > 0:
        return response.choices[0].message.content
    else:
        raise ValueError("응답 형식이 예상과 다릅니다.")
...
def extract_sql_query(text):
    # "I think final SQL QUERY:" 다음의 "SELECT"부터 ";" 앞까지 추출하는 정규 표현식
    match = re.search(r'I think final SQL QUERY:\s*(SELECT.*?);', text, re.DOTALL)
    if match:
        # 매칭된 텍스트에서 마지막 세미콜론을 제거하고 반환
        return match.group(1).strip()
    else:
        return "SELECT  " + text + " didn't extract"
...
if __name__ == '__main__':
    set_seed(10)
    spider_schema, spider_primary, spider_foreign = creatiing_schema(DATASET_SCHEMA)  # 데이터셋에서 스키마 정보를 생성합니다.
    val_df = load_data(DATASET)  # 데이터셋을 로드합니다.
    CODEX = []  # 결과를 저장할 리스트
    for index, row in val_df.iterrows():
        schema_links = None  # 스키마 링크를 저장할 변수
        while schema_links is None:
            try:
                schema_links = ollama_generation(
                    schema_linking_prompt_maker(row['question'], row['db_id']))  # 스키마 링크 생성
            except Exception as e:
                time.sleep(3)  # 3초 대기
        
        try:
            schema_links = schema_links.split("Schema_links: ")[1]  # 생성된 스키마 링크를 분리
        except Exception as e:
            schema_links = "[]"  # 기본값 설정

        classification = None  # 분류 결과를 저장할 변수
        while classification is None:
            try:
                classification = ollama_generation(
                    classification_prompt_maker(row['question'], row['db_id'], schema_links[1:]))  # 질문 분류
            except Exception as e:
                time.sleep(3)  # 3초 대기
        
        try:
            predicted_class = classification.split("Label: ")[1]  # 분류 결과에서 레이블 추출
        except Exception as e:
            predicted_class = '"NESTED"'  # 기본값으로 NESTED로 설정
        ...
        if '"EASY"' in predicted_class:
            ...
            while SQL is None:
                try:
                    SQL = ollama_generation(easy_prompt_maker(row['question'], row['db_id'], schema_links))  # EASY 쿼리 생성
                except Exception as e:
                    time.sleep(3)  # 3초 대기
            ...
            while SQL is None:
                try:
                    SQL = ollama_generation(medium_prompt_maker(row['question'], row['db_id'], schema_links))  # NON-NESTED 쿼리 생성
                except Exception as e:
                    time.sleep(3)  # 3초 대기
            try:
                SQL = SQL.split("SQL: ")[1]  # 생성된 SQL 쿼리를 분리
            except Exception as e:
                SQL = "SELECT"  # 기본값 설정
        else:
            difficulty = "NESTED"
            if 'questions = ["' in classification:
                try:
                    ...
                    #print("Error: 'questions = [\"' 문자열을 찾을 수 없습니다.")
                    sub_questions = None  # 적절한 대체 값이나 오류 처리를 추가할 수 있습니다.
            else:
                #print("Error: 'questions = [\"' 문자열이 classification에 없습니다.")
                sub_questions = None
   
            SQL = None
            while SQL is None:
                try:
                    SQL = ollama_generation(
                        hard_prompt_maker(row['question'], row['db_id'], schema_links, sub_questions))  # NESTED 쿼리 생성
                except Exception as e:
                    time.sleep(3)  # 3초 대기
            try:
                SQL = SQL.split("SQL: ")[1]  # 생성된 SQL 쿼리를 분리
        ...
        # 디버깅을 위한 프린트문 추가 (Generated SQL)
        #print(f"Generated SQL: {SQL}")

        # 디버그된 SQL 쿼리를 생성하는 과정
        debugged_SQL = None
        while debugged_SQL is None:
            try:
                debugged_SQL = ollama_debug(debuger(row['question'], row['db_id'], SQL)).replace("\n", " ")  # 생성된 SQL 쿼리 디버깅
            except Exception as e:
                time.sleep(3)  # 3초 대기
                
        
        end_time = time.time()
        sql_generation_time = end_time - start_time



        # 디버깅된 SQL 출력
        print(f"Debugged SQL: {debugged_SQL}")
        '''
        # SQL 쿼리만 추출
        extracted_SQL = extract_sql_query(debugged_SQL) 
        print(f"Final SQL: {extracted_SQL}")
        '''
        SQL = extract_sql_query(debugged_SQL)
        print(SQL)



        CODEX.append([row['question'], SQL, row['query'], row['db_id'], difficulty, sql_generation_time])  # 결과 리스트에 추가
  
        

    
    df = pd.DataFrame(CODEX, columns=['NLQ', 'PREDICTED SQL', 'GOLD SQL', 'DATABASE','DIFFICULTY', 'GENERATION TIME'])  # 결과를 데이터프레임으로 변환
    results = df[['PREDICTED SQL']].values.tolist()  # 예측된 SQL 쿼리 리스트를 생성
 
    

    
    with open(OUTPUT_FILE, 'w') as f:       
        for predicted_sql in results:
            f.write(f"{predicted_sql}\n")  # 결과를 파일에 씁니다.
    ...
    with open(TIME_OUTPUT_FILE, 'w') as time_file:
        for row in CODEX:
            question, predicted_sql, gold_sql, db_id, difficulty, gen_time = row
            time_file.write(f"Question: {question}\n")
            time_file.write(f"Generated SQL: {predicted_sql}\n")
            time_file.write(f"Difficulty Level: {difficulty}\n")
            time_file.write(f"Generation Time: {gen_time:.4f} seconds\n")
            time_file.write("-" * 50 + "\n")

# 파일 생성 여부 확인
    if os.path.exists(TIME_OUTPUT_FILE):
        print(f"시간이 성공적으로 저장되었습니다: {TIME_OUTPUT_FILE}")
    else:
        print("시간 파일이 생성되지 않았습니다.")