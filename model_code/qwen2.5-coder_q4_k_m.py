...
if sys.argv[1] == "--dataset" and sys.argv[3] == "--output":
    ...
    OUTPUT_FILE = os.path.join(script_dir, "mistral7b-instruct-v0.2.txt")  # 출력 파일 경로
    TIME_OUTPUT_FILE = os.path.join(script_dir,"mistral7b-instruct-v0.2-detail.txt")
...
def debuger(test_sample_text, database, sql):
    instruction = """#### For the given question, use the provided tables, columns, foreign keys, and primary keys to fix the given SQLite SQL QUERY for any issues. If there are any problems, fix them. If there are no issues, return the SQLite SQL QUERY as is, ending with a semicolon.
...
def generate_text(messages):
    if tokenizer.pad_token is None:
        tokenizer.pad_token = tokenizer.eos_token  # eos_token을 패딩 토큰으로 지정
...
if __name__ == '__main__':
    ...
    for index, row in val_df.iterrows():
        ...
        if '"EASY"' in predicted_class:
            ...
            while SQL is None:
                try:
                    SQL = generate_text(easy_prompt_maker(row['question'], row['db_id'], schema_links))  # EASY 쿼리 생성
                except Exception as e:
                    time.sleep(3)  # 3초 대기
            ...
            while SQL is None:
                try:
                    SQL = generate_text(medium_prompt_maker(row['question'], row['db_id'], schema_links))  # NON-NESTED 쿼리 생성
                except Exception as e:
                    time.sleep(3)  # 3초 대기
            try:
        ...
        while debugged_SQL is None:
            try:
                debugged_SQL = debug_text(debuger(row['question'], row['db_id'], SQL)).replace("\n", " ")  # 생성된 SQL 쿼리 디버깅
            except Exception as e:
                time.sleep(3)  # 3초 대기
